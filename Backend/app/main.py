...
from app.models import User, Document, QuizResponse , ActivityLog
...
async def upload_pdf(
    ...
    doc = Document(filename=file.filename, user_id=None)  # No user info
    db.add(doc)
    await db.commit()
    ...
    log = ActivityLog(user_id=None, action="upload", details=f"filename: {file.filename}")
    db.add(log)
    await db.commit()
...
async def ask_question(
    ...
    log = ActivityLog(user_id=None, action="ask", details=f"question: {question}")
    db.add(log)
    await db.commit()
...
async def generate_quiz(
    ...
    log = ActivityLog(user_id=None, action="generate_quiz", details=f"num_questions: {num_questions}")
    db.add(log)
    await db.commit()
    return {"quiz": quiz}
...
async def submit_quiz(
    ...
    log = ActivityLog(user_id=None, action="submit_quiz", details=f"score: {quiz.score}")
    db.add(log)
    await db.commit()
